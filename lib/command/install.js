...
function copy(source, target, req, cb) {
  var called = false;
  function done(err) {
  ...
  if(req.symlink) {
    try {
      if(req.force && fs.existsSync(target)) {
        fs.unlinkSync(target);  
      }
    }catch(e) {
      return done(e); 
    }
    fs.symlink(source, target, done);
  }else{
...
function installer(map, info, req, next) {
  ...
  function doCopy(source, target) {
    copy(source, target, req, function onCopy(err) {
