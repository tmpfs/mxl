...
function before(info, req, next) {
  ...
  var bypass = ['help', 'alias', 'install', 'remove'];
  ...
  for(i = 0;i < unparsed.length;i++) {
    ...
    if(dir.indexOf(constants.ALIAS_ID) === 0) {
      ...
      unparsed.splice(i, 1);
      i--;
      if(req.rc.alias[as] && req.rc.alias[as].file) {
        aliases[as] = req.rc.alias[as];
        //aliases[as].key = as;
        //req.rc.alias[as].alias = true;
        // add the alias object as a file
        files.push(req.rc.alias[as]); 
      }else{
        return next(this.errors.EALIAS_NOT_FOUND, [constants.ALIAS_ID, as]);
      }
    }
  }
  ...
  if(!dirs.length && !files.length && !alias.length) {
    ...
    }else if(fs.existsSync(path.join(process.cwd(), constants.FILENAME))) {
      files.push(path.join(process.cwd(), constants.FILENAME)); 
    }else{
      return next(
        new Error(
