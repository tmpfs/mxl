...
function attrs(cwd, opts) {
  ...
  for(var i = 0;i < att.length;i++) {
    ...
    this.log.info(' ' + prefix + ' ' + att[i].key + ' %s', att[i].value);
  }
}
...
function print(k, op) {
  ...
  switch(op) {
    case ops.show:
      this.log.info(constants.ALIAS_ID + k + ' %s', file);
      break;
    case ops.rm:
      this.log.info('-' + k + ' %s', file);
      break;
    case ops.add:
      this.log.info('+' + k + ' %s', file);
      break;
