...
function source(conf, info, req, next) {
  ...
  var cmd = [TMUX].concat('source-file', conf).join(' ');
  var cwd = req.cwd;
  var noop = req.noop;
  if(cwd === undefined) {
    cwd = path.dirname(conf); 
  }
  var id = req.key || key(conf);
  var alias = req.launch.lookup[conf];
  if(alias) {
    id = alias.id;
    // use cwd from alias when a match is found
    if(alias.cwd) {
      cwd = alias.cwd; 
    }
  }
  ...
  console.info(':source-file %s (cwd: %s, key: %s)', conf, cwd, id);
