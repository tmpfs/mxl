  ...
  for(i = 0;i < dirs.length;i++) {
    ...
    for(j = 0;j < files.length;j++) {
      ...
      if(file.alias) {
        ...
        // when using a global alias and not opening a session
        // we need to manipulate the key to prevent collisions
        if(isGlobal && !hasSession) {
          file.key = path.basename(file.cwd)
            + '/' + path.basename(path.dirname(file.alias.file));
        }

        // handle session option
        if(file.alias.options && file.alias.options.session) {
          hasSession = true; 
          req.session = file.alias.options.session;
        }
      }
      ...
      file.key = file.key || key(file.file);
...
function source(conf, info, req, next) {
  ...
  var env = {
    mxl_scratch: constants.SCRATCH,
    mxl_tpl: constants.TPL,
    mxl_key: id,
    mxl_name: name,
  ...
  exec(cmd, opts, function(err, stdout, stderr) {
    ...
    // unset environment
    envcmd = tmenv.unset(env, {exec: true, terminate: false}, req);
    exec(envcmd, next);
  })
}
