...
function getEnvCommand(env, unset) {
  var envcmd = ''
    , flag = unset ? 'gu' : 'g';
  for(var k in env) {
    envcmd += util.format(
      (unset ? ';' : '')
      + 'tmux set-environment ' + (!unset ? '-%s %s "%s"' : '-%s %s')
      + (!unset ? ';' : ''), flag, k, !unset ? env[k] : '');
  }
  return envcmd;
}
...
function source(conf, info, req, next) {
  ...
  var envcmd = getEnvCommand(env);
  opts = {env: process.env, cwd: cwd};
  cmd = envcmd + cmd;
  //cmd = envcmd + cmd + getEnvCommand(env, true);
