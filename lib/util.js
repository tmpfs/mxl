...
function search(info, req, next) {
  ...
  // recursive
  var opts = {folder: folder, file: file, absolute: true};
  if(req.recursive) {
    return find(req.launch.dirs, opts, next);
  }
...
function session(info, req, next) {
  ...
  var opts = {file: file, key: '' + req.session, system: true};
...
function source(conf, info, req, next) {
  ...
  //console.log(id + ' %s (cwd: %s)', file, cwd);
  console.info(id + ' %s (cwd: %s)', file, cwd);
