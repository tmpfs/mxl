...
function source(conf, info, req, next) {
  ...
  if(!req.directories.length && conf.alias) {
  ...
  if(req.sesskey) {
    ...
    if(!req.each) {
      env.mxl_name = (req.usecwd ? id : path.basename(path.dirname(file)));
    }

    if(id === req.sesskey) {
      id = env.mxl_name = key(file); 
    }

    // update printable id for non-session files
    if(!req.session) {
      id = req.sesskey + ':' + env.mxl_name;
    }
